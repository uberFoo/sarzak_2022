...
impl Pattern {
    ...
    pub fn r256_pattern<'a>(&'a self, store: &'a LuDogRwlockStore) -> Vec<Arc<RwLock<Pattern>>> {
        match self.next {
    ...
    pub fn r256c_pattern<'a>(&'a self, store: &'a LuDogRwlockStore) -> Vec<Arc<RwLock<Pattern>>> {
        let pattern = store
            .iter_pattern()
