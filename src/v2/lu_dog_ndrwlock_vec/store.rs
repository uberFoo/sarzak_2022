...
boolean_literal_free_list: Mutex::new(self.boolean_literal_free_list.lock().unwrap().clone()),
boolean_literal: Arc::new(RwLock::new(self.boolean_literal.read().unwrap().clone())),
boolean_operator_free_list: Mutex::new(self.boolean_operator_free_list.lock().unwrap().clone()),
...
data_structure_free_list: Mutex::new(self.data_structure_free_list.lock().unwrap().clone()),
data_structure: Arc::new(RwLock::new(self.data_structure.read().unwrap().clone())),
dwarf_source_file_free_list: Mutex::new(self.dwarf_source_file_free_list.lock().unwrap().clone()),
dwarf_source_file: Arc::new(RwLock::new(self.dwarf_source_file.read().unwrap().clone())),
...
enum_generic_type_free_list: Mutex::new(self.enum_generic_type_free_list.lock().unwrap().clone()),
enum_generic_type: Arc::new(RwLock::new(self.enum_generic_type.read().unwrap().clone())),
...
expression_bit_free_list: Mutex::new(self.expression_bit_free_list.lock().unwrap().clone()),
expression_bit: Arc::new(RwLock::new(self.expression_bit.read().unwrap().clone())),
expression_statement_free_list: Mutex::new(self.expression_statement_free_list.lock().unwrap().clone()),
expression_statement: Arc::new(RwLock::new(self.expression_statement.read().unwrap().clone())),
external_implementation_free_list: Mutex::new(self.external_implementation_free_list.lock().unwrap().clone()),
external_implementation: Arc::new(RwLock::new(self.external_implementation.read().unwrap().clone())),
...
field_access_target_free_list: Mutex::new(self.field_access_target_free_list.lock().unwrap().clone()),
field_access_target: Arc::new(RwLock::new(self.field_access_target.read().unwrap().clone())),
field_expression_free_list: Mutex::new(self.field_expression_free_list.lock().unwrap().clone()),
field_expression: Arc::new(RwLock::new(self.field_expression.read().unwrap().clone())),
float_literal_free_list: Mutex::new(self.float_literal_free_list.lock().unwrap().clone()),
...
format_string_free_list: Mutex::new(self.format_string_free_list.lock().unwrap().clone()),
...
function_call_free_list: Mutex::new(self.function_call_free_list.lock().unwrap().clone()),
...
halt_and_catch_fire_free_list: Mutex::new(self.halt_and_catch_fire_free_list.lock().unwrap().clone()),
halt_and_catch_fire: Arc::new(RwLock::new(self.halt_and_catch_fire.read().unwrap().clone())),
x_if_free_list: Mutex::new(self.x_if_free_list.lock().unwrap().clone()),
x_if: Arc::new(RwLock::new(self.x_if.read().unwrap().clone())),
implementation_block_free_list: Mutex::new(self.implementation_block_free_list.lock().unwrap().clone()),
implementation_block: Arc::new(RwLock::new(self.implementation_block.read().unwrap().clone())),
...
integer_literal_free_list: Mutex::new(self.integer_literal_free_list.lock().unwrap().clone()),
...
lambda_parameter_free_list: Mutex::new(self.lambda_parameter_free_list.lock().unwrap().clone()),
lambda_parameter: Arc::new(RwLock::new(self.lambda_parameter.read().unwrap().clone())),
let_statement_free_list: Mutex::new(self.let_statement_free_list.lock().unwrap().clone()),
...
list_expression_free_list: Mutex::new(self.list_expression_free_list.lock().unwrap().clone()),
...
local_variable_free_list: Mutex::new(self.local_variable_free_list.lock().unwrap().clone()),
...
map_expression_free_list: Mutex::new(self.map_expression_free_list.lock().unwrap().clone()),
...
named_field_expression_free_list: Mutex::new(self.named_field_expression_free_list.lock().unwrap().clone()),
named_field_expression: Arc::new(RwLock::new(self.named_field_expression.read().unwrap().clone())),
z_object_store_free_list: Mutex::new(self.z_object_store_free_list.lock().unwrap().clone()),
...
object_wrapper_free_list: Mutex::new(self.object_wrapper_free_list.lock().unwrap().clone()),
...
range_expression_free_list: Mutex::new(self.range_expression_free_list.lock().unwrap().clone()),
range_expression: Arc::new(RwLock::new(self.range_expression.read().unwrap().clone())),
result_statement_free_list: Mutex::new(self.result_statement_free_list.lock().unwrap().clone()),
...
static_method_call_free_list: Mutex::new(self.static_method_call_free_list.lock().unwrap().clone()),
static_method_call: Arc::new(RwLock::new(self.static_method_call.read().unwrap().clone())),
...
string_literal_free_list: Mutex::new(self.string_literal_free_list.lock().unwrap().clone()),
...
struct_expression_free_list: Mutex::new(self.struct_expression_free_list.lock().unwrap().clone()),
struct_expression: Arc::new(RwLock::new(self.struct_expression.read().unwrap().clone())),
...
struct_generic_free_list: Mutex::new(self.struct_generic_free_list.lock().unwrap().clone()),
...
unnamed_field_expression_free_list: Mutex::new(self.unnamed_field_expression_free_list.lock().unwrap().clone()),
unnamed_field_expression: Arc::new(RwLock::new(self.unnamed_field_expression.read().unwrap().clone())),
...
variable_expression_free_list: Mutex::new(self.variable_expression_free_list.lock().unwrap().clone()),
variable_expression: Arc::new(RwLock::new(self.variable_expression.read().unwrap().clone())),
}
}
}
impl ObjectStore {
pub fn merge(&mut self, other: &ObjectStore) {
                        ...
                        if self.argument.read().unwrap().len() != other.argument.read().unwrap().len() {
                            let mut argument = self.argument.write().unwrap();kts
                        ...
                        if self.a_wait.read().unwrap().len() != other.a_wait.read().unwrap().len() {
                            let mut a_wait = self.a_wait.write().unwrap();kts
                        ...
                        if self.binary.read().unwrap().len() != other.binary.read().unwrap().len() {
                            let mut binary = self.binary.write().unwrap();kts
                        ...
                        if self.block.read().unwrap().len() != other.block.read().unwrap().len() {
                            let mut block = self.block.write().unwrap();kts
                        ...
                        if self.body.read().unwrap().len() != other.body.read().unwrap().len() {
                            let mut body = self.body.write().unwrap();kts
                        ...
                        if self.boolean_literal.read().unwrap().len() != other.boolean_literal.read().unwrap().len() {
                            let mut boolean_literal = self.boolean_literal.write().unwrap();kts
                        ...
                        if self.boolean_operator.read().unwrap().len() != other.boolean_operator.read().unwrap().len() {
                            let mut boolean_operator = self.boolean_operator.write().unwrap();kts
                        ...
                        if self.call.read().unwrap().len() != other.call.read().unwrap().len() {
                            let mut call = self.call.write().unwrap();kts
                        ...
                        if self.char_literal.read().unwrap().len() != other.char_literal.read().unwrap().len() {
                            let mut char_literal = self.char_literal.write().unwrap();kts
                        ...
                        if self.comparison.read().unwrap().len() != other.comparison.read().unwrap().len() {
                            let mut comparison = self.comparison.write().unwrap();kts
                        ...
                        if self.data_structure.read().unwrap().len() != other.data_structure.read().unwrap().len() {
                            let mut data_structure = self.data_structure.write().unwrap();kts
                        ...
                        if self.dwarf_source_file.read().unwrap().len() != other.dwarf_source_file.read().unwrap().len() {
                            let mut dwarf_source_file = self.dwarf_source_file.write().unwrap();kts
                            other.dwarf_source_file.read().unwrap().iter().for_each(|x| {
                                if let Some(x) = x {
                        ...
                        if self.enum_field.read().unwrap().len() != other.enum_field.read().unwrap().len() {
                            let mut enum_field = self.enum_field.write().unwrap();kts
                        ...
                        if self.enum_generic.read().unwrap().len() != other.enum_generic.read().unwrap().len() {
                            let mut enum_generic = self.enum_generic.write().unwrap();kts
                        ...
                        if self.enum_generic_type.read().unwrap().len() != other.enum_generic_type.read().unwrap().len() {
                            let mut enum_generic_type = self.enum_generic_type.write().unwrap();kts
                            other.enum_generic_type.read().unwrap().iter().for_each(|x| {
                                if let Some(x) = x {
                        ...
                        if self.enumeration.read().unwrap().len() != other.enumeration.read().unwrap().len() {
                            let mut enumeration = self.enumeration.write().unwrap();kts
                        ...
                        if self.expression.read().unwrap().len() != other.expression.read().unwrap().len() {
                            let mut expression = self.expression.write().unwrap();kts
                        ...
                        if self.expression_bit.read().unwrap().len() != other.expression_bit.read().unwrap().len() {
                            let mut expression_bit = self.expression_bit.write().unwrap();kts
                        ...
                        if self.expression_statement.read().unwrap().len() != other.expression_statement.read().unwrap().len() {
                            let mut expression_statement = self.expression_statement.write().unwrap();kts
                            other.expression_statement.read().unwrap().iter().for_each(|x| {
                                if let Some(x) = x {
                        ...
                        if self.external_implementation.read().unwrap().len() != other.external_implementation.read().unwrap().len() {
                            let mut external_implementation = self.external_implementation.write().unwrap();kts
                            other.external_implementation.read().unwrap().iter().for_each(|x| {
                                if let Some(x) = x {
                        ...
                        if self.field.read().unwrap().len() != other.field.read().unwrap().len() {
                            let mut field = self.field.write().unwrap();kts
                        ...
                        if self.field_access.read().unwrap().len() != other.field_access.read().unwrap().len() {
                            let mut field_access = self.field_access.write().unwrap();kts
                        ...
                        if self.field_access_target.read().unwrap().len() != other.field_access_target.read().unwrap().len() {
                            let mut field_access_target = self.field_access_target.write().unwrap();kts
                            other.field_access_target.read().unwrap().iter().for_each(|x| {
                                if let Some(x) = x {
                        ...
                        if self.field_expression.read().unwrap().len() != other.field_expression.read().unwrap().len() {
                            let mut field_expression = self.field_expression.write().unwrap();kts
                        ...
                        if self.float_literal.read().unwrap().len() != other.float_literal.read().unwrap().len() {
                            let mut float_literal = self.float_literal.write().unwrap();kts
                        ...
                        if self.for_loop.read().unwrap().len() != other.for_loop.read().unwrap().len() {
                            let mut for_loop = self.for_loop.write().unwrap();kts
                        ...
                        if self.format_bit.read().unwrap().len() != other.format_bit.read().unwrap().len() {
                            let mut format_bit = self.format_bit.write().unwrap();kts
                        ...
                        if self.format_string.read().unwrap().len() != other.format_string.read().unwrap().len() {
                            let mut format_string = self.format_string.write().unwrap();kts
                        ...
                        if self.func_generic.read().unwrap().len() != other.func_generic.read().unwrap().len() {
                            let mut func_generic = self.func_generic.write().unwrap();kts
                        ...
                        if self.function.read().unwrap().len() != other.function.read().unwrap().len() {
                            let mut function = self.function.write().unwrap();kts
                        ...
                        if self.function_call.read().unwrap().len() != other.function_call.read().unwrap().len() {
                            let mut function_call = self.function_call.write().unwrap();kts
                        ...
                        if self.x_future.read().unwrap().len() != other.x_future.read().unwrap().len() {
                            let mut x_future = self.x_future.write().unwrap();kts
                        ...
                        if self.grouped.read().unwrap().len() != other.grouped.read().unwrap().len() {
                            let mut grouped = self.grouped.write().unwrap();kts
                        ...
                        if self.halt_and_catch_fire.read().unwrap().len() != other.halt_and_catch_fire.read().unwrap().len() {
                            let mut halt_and_catch_fire = self.halt_and_catch_fire.write().unwrap();kts
                            other.halt_and_catch_fire.read().unwrap().iter().for_each(|x| {
                                if let Some(x) = x {
                        ...
                        if self.x_if.read().unwrap().len() != other.x_if.read().unwrap().len() {
                            let mut x_if = self.x_if.write().unwrap();kts
                        ...
                        if self.implementation_block.read().unwrap().len() != other.implementation_block.read().unwrap().len() {
                            let mut implementation_block = self.implementation_block.write().unwrap();kts
                            other.implementation_block.read().unwrap().iter().for_each(|x| {
                                if let Some(x) = x {
                        ...
                        if self.import.read().unwrap().len() != other.import.read().unwrap().len() {
                            let mut import = self.import.write().unwrap();kts
                        ...
                        if self.index.read().unwrap().len() != other.index.read().unwrap().len() {
                            let mut index = self.index.write().unwrap();kts
                        ...
                        if self.integer_literal.read().unwrap().len() != other.integer_literal.read().unwrap().len() {
                            let mut integer_literal = self.integer_literal.write().unwrap();kts
                        ...
                        if self.item.read().unwrap().len() != other.item.read().unwrap().len() {
                            let mut item = self.item.write().unwrap();kts
                        ...
                        if self.lambda.read().unwrap().len() != other.lambda.read().unwrap().len() {
                            let mut lambda = self.lambda.write().unwrap();kts
                        ...
                        if self.lambda_parameter.read().unwrap().len() != other.lambda_parameter.read().unwrap().len() {
                            let mut lambda_parameter = self.lambda_parameter.write().unwrap();kts
                        ...
                        if self.let_statement.read().unwrap().len() != other.let_statement.read().unwrap().len() {
                            let mut let_statement = self.let_statement.write().unwrap();kts
                        ...
                        if self.list.read().unwrap().len() != other.list.read().unwrap().len() {
                            let mut list = self.list.write().unwrap();kts
                        ...
                        if self.list_element.read().unwrap().len() != other.list_element.read().unwrap().len() {
                            let mut list_element = self.list_element.write().unwrap();kts
                        ...
                        if self.list_expression.read().unwrap().len() != other.list_expression.read().unwrap().len() {
                            let mut list_expression = self.list_expression.write().unwrap();kts
                        ...
                        if self.literal.read().unwrap().len() != other.literal.read().unwrap().len() {
                            let mut literal = self.literal.write().unwrap();kts
                        ...
                        if self.local_variable.read().unwrap().len() != other.local_variable.read().unwrap().len() {
                            let mut local_variable = self.local_variable.write().unwrap();kts
                        ...
                        if self.x_macro.read().unwrap().len() != other.x_macro.read().unwrap().len() {
                            let mut x_macro = self.x_macro.write().unwrap();kts
                        ...
                        if self.map.read().unwrap().len() != other.map.read().unwrap().len() {
                            let mut map = self.map.write().unwrap();kts
                        ...
                        if self.map_element.read().unwrap().len() != other.map_element.read().unwrap().len() {
                            let mut map_element = self.map_element.write().unwrap();kts
                        ...
                        if self.map_expression.read().unwrap().len() != other.map_expression.read().unwrap().len() {
                            let mut map_expression = self.map_expression.write().unwrap();kts
                        ...
                        if self.x_match.read().unwrap().len() != other.x_match.read().unwrap().len() {
                            let mut x_match = self.x_match.write().unwrap();kts
                        ...
                        if self.method_call.read().unwrap().len() != other.method_call.read().unwrap().len() {
                            let mut method_call = self.method_call.write().unwrap();kts
                        ...
                        if self.named_field_expression.read().unwrap().len() != other.named_field_expression.read().unwrap().len() {
                            let mut named_field_expression = self.named_field_expression.write().unwrap();kts
                            other.named_field_expression.read().unwrap().iter().for_each(|x| {
                                if let Some(x) = x {
                        ...
                        if self.z_object_store.read().unwrap().len() != other.z_object_store.read().unwrap().len() {
                            let mut z_object_store = self.z_object_store.write().unwrap();kts
                        ...
                        if self.object_wrapper.read().unwrap().len() != other.object_wrapper.read().unwrap().len() {
                            let mut object_wrapper = self.object_wrapper.write().unwrap();kts
                        ...
                        if self.operator.read().unwrap().len() != other.operator.read().unwrap().len() {
                            let mut operator = self.operator.write().unwrap();kts
                        ...
                        if self.parameter.read().unwrap().len() != other.parameter.read().unwrap().len() {
                            let mut parameter = self.parameter.write().unwrap();kts
                        ...
                        if self.x_path.read().unwrap().len() != other.x_path.read().unwrap().len() {
                            let mut x_path = self.x_path.write().unwrap();kts
                        ...
                        if self.path_element.read().unwrap().len() != other.path_element.read().unwrap().len() {
                            let mut path_element = self.path_element.write().unwrap();kts
                        ...
                        if self.pattern.read().unwrap().len() != other.pattern.read().unwrap().len() {
                            let mut pattern = self.pattern.write().unwrap();kts
                        ...
                        if self.x_plugin.read().unwrap().len() != other.x_plugin.read().unwrap().len() {
                            let mut x_plugin = self.x_plugin.write().unwrap();kts
                        ...
                        if self.x_print.read().unwrap().len() != other.x_print.read().unwrap().len() {
                            let mut x_print = self.x_print.write().unwrap();kts
                        ...
                        if self.range_expression.read().unwrap().len() != other.range_expression.read().unwrap().len() {
                            let mut range_expression = self.range_expression.write().unwrap();kts
                        ...
                        if self.result_statement.read().unwrap().len() != other.result_statement.read().unwrap().len() {
                            let mut result_statement = self.result_statement.write().unwrap();kts
                        ...
                        if self.x_return.read().unwrap().len() != other.x_return.read().unwrap().len() {
                            let mut x_return = self.x_return.write().unwrap();kts
                        ...
                        if self.span.read().unwrap().len() != other.span.read().unwrap().len() {
                            let mut span = self.span.write().unwrap();kts
                        ...
                        if self.statement.read().unwrap().len() != other.statement.read().unwrap().len() {
                            let mut statement = self.statement.write().unwrap();kts
                        ...
                        if self.static_method_call.read().unwrap().len() != other.static_method_call.read().unwrap().len() {
                            let mut static_method_call = self.static_method_call.write().unwrap();kts
                            other.static_method_call.read().unwrap().iter().for_each(|x| {
                                if let Some(x) = x {
                        ...
                        if self.string_bit.read().unwrap().len() != other.string_bit.read().unwrap().len() {
                            let mut string_bit = self.string_bit.write().unwrap();kts
                        ...
                        if self.string_literal.read().unwrap().len() != other.string_literal.read().unwrap().len() {
                            let mut string_literal = self.string_literal.write().unwrap();kts
                        ...
                        if self.woog_struct.read().unwrap().len() != other.woog_struct.read().unwrap().len() {
                            let mut woog_struct = self.woog_struct.write().unwrap();kts
                        ...
                        if self.struct_expression.read().unwrap().len() != other.struct_expression.read().unwrap().len() {
                            let mut struct_expression = self.struct_expression.write().unwrap();kts
                            other.struct_expression.read().unwrap().iter().for_each(|x| {
                                if let Some(x) = x {
                        ...
                        if self.struct_field.read().unwrap().len() != other.struct_field.read().unwrap().len() {
                            let mut struct_field = self.struct_field.write().unwrap();kts
                        ...
                        if self.struct_generic.read().unwrap().len() != other.struct_generic.read().unwrap().len() {
                            let mut struct_generic = self.struct_generic.write().unwrap();kts
                        ...
                        if self.tuple_field.read().unwrap().len() != other.tuple_field.read().unwrap().len() {
                            let mut tuple_field = self.tuple_field.write().unwrap();kts
                        ...
                        if self.type_cast.read().unwrap().len() != other.type_cast.read().unwrap().len() {
                            let mut type_cast = self.type_cast.write().unwrap();kts
                        ...
                        if self.unary.read().unwrap().len() != other.unary.read().unwrap().len() {
                            let mut unary = self.unary.write().unwrap();kts
                        ...
                        if self.unit.read().unwrap().len() != other.unit.read().unwrap().len() {
                            let mut unit = self.unit.write().unwrap();kts
                        ...
                        if self.unnamed_field_expression.read().unwrap().len() != other.unnamed_field_expression.read().unwrap().len() {
                            let mut unnamed_field_expression = self.unnamed_field_expression.write().unwrap();kts
                            other.unnamed_field_expression.read().unwrap().iter().for_each(|x| {
                                if let Some(x) = x {
                        ...
                        if self.x_value.read().unwrap().len() != other.x_value.read().unwrap().len() {
                            let mut x_value = self.x_value.write().unwrap();kts
                        ...
                        if self.value_type.read().unwrap().len() != other.value_type.read().unwrap().len() {
                            let mut value_type = self.value_type.write().unwrap();kts
                        ...
                        if self.variable.read().unwrap().len() != other.variable.read().unwrap().len() {
                            let mut variable = self.variable.write().unwrap();kts
                        ...
                        if self.variable_expression.read().unwrap().len() != other.variable_expression.read().unwrap().len() {
                            let mut variable_expression = self.variable_expression.write().unwrap();kts
                            other.variable_expression.read().unwrap().iter().for_each(|x| {
                                if let Some(x) = x {
...
///
#[inline]
pub fn inter_argument<F>(&mut self, argument: F) -> Arc<RwLock<Argument>> where F: Fn(usize) -> Arc<RwLock<Argument>>, {
...
(0..len).filter(|i| self.argument.read().unwrap()[*i].is_some()).map(move|i|{self.argument.read().unwrap()[i].as_ref().map(|argument| argument.clone()).unwrap()})
}

/// Inter (insert) [`AWait`] into the store.
///
#[inline]
pub fn inter_a_wait<F>(&mut self, a_wait: F) -> Arc<RwLock<AWait>> where F: Fn(usize) -> Arc<RwLock<AWait>>, {
...
(0..len).filter(|i| self.a_wait.read().unwrap()[*i].is_some()).map(move|i|{self.a_wait.read().unwrap()[i].as_ref().map(|a_wait| a_wait.clone()).unwrap()})
}

/// Inter (insert) [`Binary`] into the store.
///
#[inline]
pub fn inter_binary<F>(&mut self, binary: F) -> Arc<RwLock<Binary>> where F: Fn(usize) -> Arc<RwLock<Binary>>, {
...
(0..len).filter(|i| self.binary.read().unwrap()[*i].is_some()).map(move|i|{self.binary.read().unwrap()[i].as_ref().map(|binary| binary.clone()).unwrap()})
}

/// Inter (insert) [`Block`] into the store.
///
#[inline]
pub fn inter_block<F>(&mut self, block: F) -> Arc<RwLock<Block>> where F: Fn(usize) -> Arc<RwLock<Block>>, {
...
(0..len).filter(|i| self.block.read().unwrap()[*i].is_some()).map(move|i|{self.block.read().unwrap()[i].as_ref().map(|block| block.clone()).unwrap()})
}

/// Inter (insert) [`Body`] into the store.
///
#[inline]
pub fn inter_body<F>(&mut self, body: F) -> Arc<RwLock<Body>> where F: Fn(usize) -> Arc<RwLock<Body>>, {
...
(0..len).filter(|i| self.body.read().unwrap()[*i].is_some()).map(move|i|{self.body.read().unwrap()[i].as_ref().map(|body| body.clone()).unwrap()})
}

/// Inter (insert) [`BooleanLiteral`] into the store.
///
#[inline]
pub fn inter_boolean_literal<F>(&mut self, boolean_literal: F) -> Arc<RwLock<BooleanLiteral>> where F: Fn(usize) -> Arc<RwLock<BooleanLiteral>>, {
...
let found = if let Some(boolean_literal) = self.boolean_literal.read().unwrap().iter().find(|stored| {
if let Some(stored) = stored {
...
(0..len).filter(|i| self.boolean_literal.read().unwrap()[*i].is_some()).map(move|i|{self.boolean_literal.read().unwrap()[i].as_ref().map(|boolean_literal| boolean_literal.clone()).unwrap()})
}

/// Inter (insert) [`BooleanOperator`] into the store.
///
#[inline]
pub fn inter_boolean_operator<F>(&mut self, boolean_operator: F) -> Arc<RwLock<BooleanOperator>> where F: Fn(usize) -> Arc<RwLock<BooleanOperator>>, {
...
let found = if let Some(boolean_operator) = self.boolean_operator.read().unwrap().iter().find(|stored| {
if let Some(stored) = stored {
...
///
#[inline]
pub fn exorcise_boolean_operator(&mut self, id: &usize) -> Option<Arc<RwLock<BooleanOperator>>> {
...
(0..len).filter(|i| self.boolean_operator.read().unwrap()[*i].is_some()).map(move|i|{self.boolean_operator.read().unwrap()[i].as_ref().map(|boolean_operator| boolean_operator.clone()).unwrap()})
}

/// Inter (insert) [`Call`] into the store.
///
#[inline]
pub fn inter_call<F>(&mut self, call: F) -> Arc<RwLock<Call>> where F: Fn(usize) -> Arc<RwLock<Call>>, {
...
(0..len).filter(|i| self.call.read().unwrap()[*i].is_some()).map(move|i|{self.call.read().unwrap()[i].as_ref().map(|call| call.clone()).unwrap()})
}

/// Inter (insert) [`CharLiteral`] into the store.
///
#[inline]
pub fn inter_char_literal<F>(&mut self, char_literal: F) -> Arc<RwLock<CharLiteral>> where F: Fn(usize) -> Arc<RwLock<CharLiteral>>, {
...
let found = if let Some(char_literal) = self.char_literal.read().unwrap().iter().find(|stored| {
if let Some(stored) = stored {
...
(0..len).filter(|i| self.char_literal.read().unwrap()[*i].is_some()).map(move|i|{self.char_literal.read().unwrap()[i].as_ref().map(|char_literal| char_literal.clone()).unwrap()})
}

/// Inter (insert) [`Comparison`] into the store.
///
#[inline]
pub fn inter_comparison<F>(&mut self, comparison: F) -> Arc<RwLock<Comparison>> where F: Fn(usize) -> Arc<RwLock<Comparison>>, {
...
let found = if let Some(comparison) = self.comparison.read().unwrap().iter().find(|stored| {
if let Some(stored) = stored {
...
(0..len).filter(|i| self.comparison.read().unwrap()[*i].is_some()).map(move|i|{self.comparison.read().unwrap()[i].as_ref().map(|comparison| comparison.clone()).unwrap()})
}

/// Inter (insert) [`DataStructure`] into the store.
///
#[inline]
pub fn inter_data_structure<F>(&mut self, data_structure: F) -> Arc<RwLock<DataStructure>> where F: Fn(usize) -> Arc<RwLock<DataStructure>>, {
...
let found = if let Some(data_structure) = self.data_structure.read().unwrap().iter().find(|stored| {
if let Some(stored) = stored {
...
(0..len).filter(|i| self.data_structure.read().unwrap()[*i].is_some()).map(move|i|{self.data_structure.read().unwrap()[i].as_ref().map(|data_structure| data_structure.clone()).unwrap()})
}

/// Inter (insert) [`DwarfSourceFile`] into the store.
///
#[inline]
pub fn inter_dwarf_source_file<F>(&mut self, dwarf_source_file: F) -> Arc<RwLock<DwarfSourceFile>> where F: Fn(usize) -> Arc<RwLock<DwarfSourceFile>>, {
...
let found = if let Some(dwarf_source_file) = self.dwarf_source_file.read().unwrap().iter().find(|stored| {
if let Some(stored) = stored {
...
self.dwarf_source_file_free_list.lock().unwrap().push(_index);
dwarf_source_file.clone()
} else {
...
///
#[inline]
pub fn exorcise_dwarf_source_file(&mut self, id: &usize) -> Option<Arc<RwLock<DwarfSourceFile>>> {
...
///
#[inline]
pub fn iter_dwarf_source_file(&self) -> impl Iterator<Item = Arc<RwLock<DwarfSourceFile>>> + '_ {
let len = self.dwarf_source_file.read().unwrap().len();
(0..len).filter(|i| self.dwarf_source_file.read().unwrap()[*i].is_some()).map(move|i|{self.dwarf_source_file.read().unwrap()[i].as_ref().map(|dwarf_source_file| dwarf_source_file.clone()).unwrap()})
}

/// Inter (insert) [`EnumField`] into the store.
///
#[inline]
pub fn inter_enum_field<F>(&mut self, enum_field: F) -> Arc<RwLock<EnumField>> where F: Fn(usize) -> Arc<RwLock<EnumField>>, {
...
let found = if let Some(enum_field) = self.enum_field.read().unwrap().iter().find(|stored| {
if let Some(stored) = stored {
...
(0..len).filter(|i| self.enum_field.read().unwrap()[*i].is_some()).map(move|i|{self.enum_field.read().unwrap()[i].as_ref().map(|enum_field| enum_field.clone()).unwrap()})
}

/// Inter (insert) [`EnumGeneric`] into the store.
///
#[inline]
pub fn inter_enum_generic<F>(&mut self, enum_generic: F) -> Arc<RwLock<EnumGeneric>> where F: Fn(usize) -> Arc<RwLock<EnumGeneric>>, {
...
let found = if let Some(enum_generic) = self.enum_generic.read().unwrap().iter().find(|stored| {
if let Some(stored) = stored {
...
(0..len).filter(|i| self.enum_generic.read().unwrap()[*i].is_some()).map(move|i|{self.enum_generic.read().unwrap()[i].as_ref().map(|enum_generic| enum_generic.clone()).unwrap()})
}

/// Inter (insert) [`EnumGenericType`] into the store.
///
#[inline]
pub fn inter_enum_generic_type<F>(&mut self, enum_generic_type: F) -> Arc<RwLock<EnumGenericType>> where F: Fn(usize) -> Arc<RwLock<EnumGenericType>>, {
...
let found = if let Some(enum_generic_type) = self.enum_generic_type.read().unwrap().iter().find(|stored| {
if let Some(stored) = stored {
...
self.enum_generic_type_free_list.lock().unwrap().push(_index);
enum_generic_type.clone()
} else {
...
///
#[inline]
pub fn exorcise_enum_generic_type(&mut self, id: &usize) -> Option<Arc<RwLock<EnumGenericType>>> {
...
///
#[inline]
pub fn iter_enum_generic_type(&self) -> impl Iterator<Item = Arc<RwLock<EnumGenericType>>> + '_ {
let len = self.enum_generic_type.read().unwrap().len();
(0..len).filter(|i| self.enum_generic_type.read().unwrap()[*i].is_some()).map(move|i|{self.enum_generic_type.read().unwrap()[i].as_ref().map(|enum_generic_type| enum_generic_type.clone()).unwrap()})
}

/// Inter (insert) [`Enumeration`] into the store.
///
#[inline]
pub fn inter_enumeration<F>(&mut self, enumeration: F) -> Arc<RwLock<Enumeration>> where F: Fn(usize) -> Arc<RwLock<Enumeration>>, {
...
let found = if let Some(enumeration) = self.enumeration.read().unwrap().iter().find(|stored| {
if let Some(stored) = stored {
...
;
self.enumeration_id_by_name.write().unwrap().insert(enumeration.read().unwrap().name.to_owned(), enumeration.read().unwrap().id);
enumeration
}
...
self.enumeration_id_by_name.read().unwrap().get(name).map(|enumeration| *enumeration)
}
...
(0..len).filter(|i| self.enumeration.read().unwrap()[*i].is_some()).map(move|i|{self.enumeration.read().unwrap()[i].as_ref().map(|enumeration| enumeration.clone()).unwrap()})
}

/// Inter (insert) [`Expression`] into the store.
///
#[inline]
pub fn inter_expression<F>(&mut self, expression: F) -> Arc<RwLock<Expression>> where F: Fn(usize) -> Arc<RwLock<Expression>>, {
...
let found = if let Some(expression) = self.expression.read().unwrap().iter().find(|stored| {
if let Some(stored) = stored {
...
(0..len).filter(|i| self.expression.read().unwrap()[*i].is_some()).map(move|i|{self.expression.read().unwrap()[i].as_ref().map(|expression| expression.clone()).unwrap()})
}

/// Inter (insert) [`ExpressionBit`] into the store.
///
#[inline]
pub fn inter_expression_bit<F>(&mut self, expression_bit: F) -> Arc<RwLock<ExpressionBit>> where F: Fn(usize) -> Arc<RwLock<ExpressionBit>>, {
...
let found = if let Some(expression_bit) = self.expression_bit.read().unwrap().iter().find(|stored| {
if let Some(stored) = stored {
...
(0..len).filter(|i| self.expression_bit.read().unwrap()[*i].is_some()).map(move|i|{self.expression_bit.read().unwrap()[i].as_ref().map(|expression_bit| expression_bit.clone()).unwrap()})
}

/// Inter (insert) [`ExpressionStatement`] into the store.
///
#[inline]
pub fn inter_expression_statement<F>(&mut self, expression_statement: F) -> Arc<RwLock<ExpressionStatement>> where F: Fn(usize) -> Arc<RwLock<ExpressionStatement>>, {
...
let found = if let Some(expression_statement) = self.expression_statement.read().unwrap().iter().find(|stored| {
if let Some(stored) = stored {
...
self.expression_statement_free_list.lock().unwrap().push(_index);
expression_statement.clone()
} else {
...
///
#[inline]
pub fn exhume_expression_statement(&self, id: &usize) -> Option<Arc<RwLock<ExpressionStatement>>> {
...
///
#[inline]
pub fn exorcise_expression_statement(&mut self, id: &usize) -> Option<Arc<RwLock<ExpressionStatement>>> {
...
self.expression_statement_free_list.lock().unwrap().push(*id);
result
}
...
///
#[inline]
pub fn iter_expression_statement(&self) -> impl Iterator<Item = Arc<RwLock<ExpressionStatement>>> + '_ {
let len = self.expression_statement.read().unwrap().len();
(0..len).filter(|i| self.expression_statement.read().unwrap()[*i].is_some()).map(move|i|{self.expression_statement.read().unwrap()[i].as_ref().map(|expression_statement| expression_statement.clone()).unwrap()})
}

/// Inter (insert) [`ExternalImplementation`] into the store.
///
#[inline]
pub fn inter_external_implementation<F>(&mut self, external_implementation: F) -> Arc<RwLock<ExternalImplementation>> where F: Fn(usize) -> Arc<RwLock<ExternalImplementation>>, {
let _index = if let Some(_index) = self.external_implementation_free_list.lock().unwrap().pop() {
tracing::trace!(target: "store", "recycling block {_index}.");
_index
} else {
...
let found = if let Some(external_implementation) = self.external_implementation.read().unwrap().iter().find(|stored| {
if let Some(stored) = stored {
...
self.external_implementation_free_list.lock().unwrap().push(_index);
external_implementation.clone()
} else {
tracing::debug!(target: "store", "interring {external_implementation:?}.");
self.external_implementation.write().unwrap()[_index] = Some(external_implementation.clone());
...
///
#[inline]
pub fn exhume_external_implementation(&self, id: &usize) -> Option<Arc<RwLock<ExternalImplementation>>> {
...
///
#[inline]
pub fn exorcise_external_implementation(&mut self, id: &usize) -> Option<Arc<RwLock<ExternalImplementation>>> {
...
self.external_implementation_free_list.lock().unwrap().push(*id);
result
}
...
///
#[inline]
pub fn iter_external_implementation(&self) -> impl Iterator<Item = Arc<RwLock<ExternalImplementation>>> + '_ {
let len = self.external_implementation.read().unwrap().len();
(0..len).filter(|i| self.external_implementation.read().unwrap()[*i].is_some()).map(move|i|{self.external_implementation.read().unwrap()[i].as_ref().map(|external_implementation| external_implementation.clone()).unwrap()})
}

/// Inter (insert) [`Field`] into the store.
///
#[inline]
pub fn inter_field<F>(&mut self, field: F) -> Arc<RwLock<Field>> where F: Fn(usize) -> Arc<RwLock<Field>>, {
...
;
self.field_id_by_name.write().unwrap().insert(field.read().unwrap().name.to_owned(), field.read().unwrap().id);
field
}
...
self.field_id_by_name.read().unwrap().get(name).map(|field| *field)
}
...
(0..len).filter(|i| self.field.read().unwrap()[*i].is_some()).map(move|i|{self.field.read().unwrap()[i].as_ref().map(|field| field.clone()).unwrap()})
}

/// Inter (insert) [`FieldAccess`] into the store.
///
#[inline]
pub fn inter_field_access<F>(&mut self, field_access: F) -> Arc<RwLock<FieldAccess>> where F: Fn(usize) -> Arc<RwLock<FieldAccess>>, {
...
let found = if let Some(field_access) = self.field_access.read().unwrap().iter().find(|stored| {
if let Some(stored) = stored {
...
(0..len).filter(|i| self.field_access.read().unwrap()[*i].is_some()).map(move|i|{self.field_access.read().unwrap()[i].as_ref().map(|field_access| field_access.clone()).unwrap()})
}

/// Inter (insert) [`FieldAccessTarget`] into the store.
///
#[inline]
pub fn inter_field_access_target<F>(&mut self, field_access_target: F) -> Arc<RwLock<FieldAccessTarget>> where F: Fn(usize) -> Arc<RwLock<FieldAccessTarget>>, {
...
let found = if let Some(field_access_target) = self.field_access_target.read().unwrap().iter().find(|stored| {
if let Some(stored) = stored {
...
self.field_access_target_free_list.lock().unwrap().push(_index);
field_access_target.clone()
} else {
...
///
#[inline]
pub fn exorcise_field_access_target(&mut self, id: &usize) -> Option<Arc<RwLock<FieldAccessTarget>>> {
...
///
#[inline]
pub fn iter_field_access_target(&self) -> impl Iterator<Item = Arc<RwLock<FieldAccessTarget>>> + '_ {
let len = self.field_access_target.read().unwrap().len();
(0..len).filter(|i| self.field_access_target.read().unwrap()[*i].is_some()).map(move|i|{self.field_access_target.read().unwrap()[i].as_ref().map(|field_access_target| field_access_target.clone()).unwrap()})
}

/// Inter (insert) [`FieldExpression`] into the store.
///
#[inline]
pub fn inter_field_expression<F>(&mut self, field_expression: F) -> Arc<RwLock<FieldExpression>> where F: Fn(usize) -> Arc<RwLock<FieldExpression>>, {
...
let found = if let Some(field_expression) = self.field_expression.read().unwrap().iter().find(|stored| {
if let Some(stored) = stored {
...
///
#[inline]
pub fn exorcise_field_expression(&mut self, id: &usize) -> Option<Arc<RwLock<FieldExpression>>> {
...
(0..len).filter(|i| self.field_expression.read().unwrap()[*i].is_some()).map(move|i|{self.field_expression.read().unwrap()[i].as_ref().map(|field_expression| field_expression.clone()).unwrap()})
}

/// Inter (insert) [`FloatLiteral`] into the store.
///
#[inline]
pub fn inter_float_literal<F>(&mut self, float_literal: F) -> Arc<RwLock<FloatLiteral>> where F: Fn(usize) -> Arc<RwLock<FloatLiteral>>, {
...
let found = if let Some(float_literal) = self.float_literal.read().unwrap().iter().find(|stored| {
if let Some(stored) = stored {
...
(0..len).filter(|i| self.float_literal.read().unwrap()[*i].is_some()).map(move|i|{self.float_literal.read().unwrap()[i].as_ref().map(|float_literal| float_literal.clone()).unwrap()})
}

/// Inter (insert) [`ForLoop`] into the store.
///
#[inline]
pub fn inter_for_loop<F>(&mut self, for_loop: F) -> Arc<RwLock<ForLoop>> where F: Fn(usize) -> Arc<RwLock<ForLoop>>, {
...
(0..len).filter(|i| self.for_loop.read().unwrap()[*i].is_some()).map(move|i|{self.for_loop.read().unwrap()[i].as_ref().map(|for_loop| for_loop.clone()).unwrap()})
}

/// Inter (insert) [`FormatBit`] into the store.
///
#[inline]
pub fn inter_format_bit<F>(&mut self, format_bit: F) -> Arc<RwLock<FormatBit>> where F: Fn(usize) -> Arc<RwLock<FormatBit>>, {
...
let found = if let Some(format_bit) = self.format_bit.read().unwrap().iter().find(|stored| {
if let Some(stored) = stored {
...
(0..len).filter(|i| self.format_bit.read().unwrap()[*i].is_some()).map(move|i|{self.format_bit.read().unwrap()[i].as_ref().map(|format_bit| format_bit.clone()).unwrap()})
}

/// Inter (insert) [`FormatString`] into the store.
///
#[inline]
pub fn inter_format_string<F>(&mut self, format_string: F) -> Arc<RwLock<FormatString>> where F: Fn(usize) -> Arc<RwLock<FormatString>>, {
...
let found = if let Some(format_string) = self.format_string.read().unwrap().iter().find(|stored| {
if let Some(stored) = stored {
...
(0..len).filter(|i| self.format_string.read().unwrap()[*i].is_some()).map(move|i|{self.format_string.read().unwrap()[i].as_ref().map(|format_string| format_string.clone()).unwrap()})
}

/// Inter (insert) [`FuncGeneric`] into the store.
///
#[inline]
pub fn inter_func_generic<F>(&mut self, func_generic: F) -> Arc<RwLock<FuncGeneric>> where F: Fn(usize) -> Arc<RwLock<FuncGeneric>>, {
...
let found = if let Some(func_generic) = self.func_generic.read().unwrap().iter().find(|stored| {
if let Some(stored) = stored {
...
(0..len).filter(|i| self.func_generic.read().unwrap()[*i].is_some()).map(move|i|{self.func_generic.read().unwrap()[i].as_ref().map(|func_generic| func_generic.clone()).unwrap()})
}

/// Inter (insert) [`Function`] into the store.
///
#[inline]
pub fn inter_function<F>(&mut self, function: F) -> Arc<RwLock<Function>> where F: Fn(usize) -> Arc<RwLock<Function>>, {
...
;
self.function_id_by_name.write().unwrap().insert(function.read().unwrap().name.to_owned(), function.read().unwrap().id);
function
}
...
self.function_id_by_name.read().unwrap().get(name).map(|function| *function)
}
...
(0..len).filter(|i| self.function.read().unwrap()[*i].is_some()).map(move|i|{self.function.read().unwrap()[i].as_ref().map(|function| function.clone()).unwrap()})
}

/// Inter (insert) [`FunctionCall`] into the store.
///
#[inline]
pub fn inter_function_call<F>(&mut self, function_call: F) -> Arc<RwLock<FunctionCall>> where F: Fn(usize) -> Arc<RwLock<FunctionCall>>, {
...
let found = if let Some(function_call) = self.function_call.read().unwrap().iter().find(|stored| {
if let Some(stored) = stored {
...
(0..len).filter(|i| self.function_call.read().unwrap()[*i].is_some()).map(move|i|{self.function_call.read().unwrap()[i].as_ref().map(|function_call| function_call.clone()).unwrap()})
}

/// Inter (insert) [`XFuture`] into the store.
///
#[inline]
pub fn inter_x_future<F>(&mut self, x_future: F) -> Arc<RwLock<XFuture>> where F: Fn(usize) -> Arc<RwLock<XFuture>>, {
...
(0..len).filter(|i| self.x_future.read().unwrap()[*i].is_some()).map(move|i|{self.x_future.read().unwrap()[i].as_ref().map(|x_future| x_future.clone()).unwrap()})
}

/// Inter (insert) [`Grouped`] into the store.
///
#[inline]
pub fn inter_grouped<F>(&mut self, grouped: F) -> Arc<RwLock<Grouped>> where F: Fn(usize) -> Arc<RwLock<Grouped>>, {
...
(0..len).filter(|i| self.grouped.read().unwrap()[*i].is_some()).map(move|i|{self.grouped.read().unwrap()[i].as_ref().map(|grouped| grouped.clone()).unwrap()})
}

/// Inter (insert) [`HaltAndCatchFire`] into the store.
///
#[inline]
pub fn inter_halt_and_catch_fire<F>(&mut self, halt_and_catch_fire: F) -> Arc<RwLock<HaltAndCatchFire>> where F: Fn(usize) -> Arc<RwLock<HaltAndCatchFire>>, {
...
let found = if let Some(halt_and_catch_fire) = self.halt_and_catch_fire.read().unwrap().iter().find(|stored| {
if let Some(stored) = stored {
...
self.halt_and_catch_fire_free_list.lock().unwrap().push(_index);
halt_and_catch_fire.clone()
} else {
...
///
#[inline]
pub fn exorcise_halt_and_catch_fire(&mut self, id: &usize) -> Option<Arc<RwLock<HaltAndCatchFire>>> {
...
///
#[inline]
pub fn iter_halt_and_catch_fire(&self) -> impl Iterator<Item = Arc<RwLock<HaltAndCatchFire>>> + '_ {
let len = self.halt_and_catch_fire.read().unwrap().len();
(0..len).filter(|i| self.halt_and_catch_fire.read().unwrap()[*i].is_some()).map(move|i|{self.halt_and_catch_fire.read().unwrap()[i].as_ref().map(|halt_and_catch_fire| halt_and_catch_fire.clone()).unwrap()})
}

/// Inter (insert) [`XIf`] into the store.
///
#[inline]
pub fn inter_x_if<F>(&mut self, x_if: F) -> Arc<RwLock<XIf>> where F: Fn(usize) -> Arc<RwLock<XIf>>, {
...
(0..len).filter(|i| self.x_if.read().unwrap()[*i].is_some()).map(move|i|{self.x_if.read().unwrap()[i].as_ref().map(|x_if| x_if.clone()).unwrap()})
}

/// Inter (insert) [`ImplementationBlock`] into the store.
///
#[inline]
pub fn inter_implementation_block<F>(&mut self, implementation_block: F) -> Arc<RwLock<ImplementationBlock>> where F: Fn(usize) -> Arc<RwLock<ImplementationBlock>>, {
...
let found = if let Some(implementation_block) = self.implementation_block.read().unwrap().iter().find(|stored| {
if let Some(stored) = stored {
...
self.implementation_block_free_list.lock().unwrap().push(_index);
implementation_block.clone()
} else {
...
///
#[inline]
pub fn exhume_implementation_block(&self, id: &usize) -> Option<Arc<RwLock<ImplementationBlock>>> {
...
///
#[inline]
pub fn exorcise_implementation_block(&mut self, id: &usize) -> Option<Arc<RwLock<ImplementationBlock>>> {
...
self.implementation_block_free_list.lock().unwrap().push(*id);
result
}
...
///
#[inline]
pub fn iter_implementation_block(&self) -> impl Iterator<Item = Arc<RwLock<ImplementationBlock>>> + '_ {
let len = self.implementation_block.read().unwrap().len();
(0..len).filter(|i| self.implementation_block.read().unwrap()[*i].is_some()).map(move|i|{self.implementation_block.read().unwrap()[i].as_ref().map(|implementation_block| implementation_block.clone()).unwrap()})
}

/// Inter (insert) [`Import`] into the store.
///
#[inline]
pub fn inter_import<F>(&mut self, import: F) -> Arc<RwLock<Import>> where F: Fn(usize) -> Arc<RwLock<Import>>, {
...
(0..len).filter(|i| self.import.read().unwrap()[*i].is_some()).map(move|i|{self.import.read().unwrap()[i].as_ref().map(|import| import.clone()).unwrap()})
}

/// Inter (insert) [`Index`] into the store.
///
#[inline]
pub fn inter_index<F>(&mut self, index: F) -> Arc<RwLock<Index>> where F: Fn(usize) -> Arc<RwLock<Index>>, {
...
(0..len).filter(|i| self.index.read().unwrap()[*i].is_some()).map(move|i|{self.index.read().unwrap()[i].as_ref().map(|index| index.clone()).unwrap()})
}

/// Inter (insert) [`IntegerLiteral`] into the store.
///
#[inline]
pub fn inter_integer_literal<F>(&mut self, integer_literal: F) -> Arc<RwLock<IntegerLiteral>> where F: Fn(usize) -> Arc<RwLock<IntegerLiteral>>, {
...
let found = if let Some(integer_literal) = self.integer_literal.read().unwrap().iter().find(|stored| {
if let Some(stored) = stored {
...
(0..len).filter(|i| self.integer_literal.read().unwrap()[*i].is_some()).map(move|i|{self.integer_literal.read().unwrap()[i].as_ref().map(|integer_literal| integer_literal.clone()).unwrap()})
}

/// Inter (insert) [`Item`] into the store.
///
#[inline]
pub fn inter_item<F>(&mut self, item: F) -> Arc<RwLock<Item>> where F: Fn(usize) -> Arc<RwLock<Item>>, {
...
(0..len).filter(|i| self.item.read().unwrap()[*i].is_some()).map(move|i|{self.item.read().unwrap()[i].as_ref().map(|item| item.clone()).unwrap()})
}

/// Inter (insert) [`Lambda`] into the store.
///
#[inline]
pub fn inter_lambda<F>(&mut self, lambda: F) -> Arc<RwLock<Lambda>> where F: Fn(usize) -> Arc<RwLock<Lambda>>, {
...
(0..len).filter(|i| self.lambda.read().unwrap()[*i].is_some()).map(move|i|{self.lambda.read().unwrap()[i].as_ref().map(|lambda| lambda.clone()).unwrap()})
}

/// Inter (insert) [`LambdaParameter`] into the store.
///
#[inline]
pub fn inter_lambda_parameter<F>(&mut self, lambda_parameter: F) -> Arc<RwLock<LambdaParameter>> where F: Fn(usize) -> Arc<RwLock<LambdaParameter>>, {
...
let found = if let Some(lambda_parameter) = self.lambda_parameter.read().unwrap().iter().find(|stored| {
if let Some(stored) = stored {
...
///
#[inline]
pub fn exorcise_lambda_parameter(&mut self, id: &usize) -> Option<Arc<RwLock<LambdaParameter>>> {
...
(0..len).filter(|i| self.lambda_parameter.read().unwrap()[*i].is_some()).map(move|i|{self.lambda_parameter.read().unwrap()[i].as_ref().map(|lambda_parameter| lambda_parameter.clone()).unwrap()})
}

/// Inter (insert) [`LetStatement`] into the store.
///
#[inline]
pub fn inter_let_statement<F>(&mut self, let_statement: F) -> Arc<RwLock<LetStatement>> where F: Fn(usize) -> Arc<RwLock<LetStatement>>, {
...
let found = if let Some(let_statement) = self.let_statement.read().unwrap().iter().find(|stored| {
if let Some(stored) = stored {
...
(0..len).filter(|i| self.let_statement.read().unwrap()[*i].is_some()).map(move|i|{self.let_statement.read().unwrap()[i].as_ref().map(|let_statement| let_statement.clone()).unwrap()})
}

/// Inter (insert) [`List`] into the store.
///
#[inline]
pub fn inter_list<F>(&mut self, list: F) -> Arc<RwLock<List>> where F: Fn(usize) -> Arc<RwLock<List>>, {
...
(0..len).filter(|i| self.list.read().unwrap()[*i].is_some()).map(move|i|{self.list.read().unwrap()[i].as_ref().map(|list| list.clone()).unwrap()})
}

/// Inter (insert) [`ListElement`] into the store.
///
#[inline]
pub fn inter_list_element<F>(&mut self, list_element: F) -> Arc<RwLock<ListElement>> where F: Fn(usize) -> Arc<RwLock<ListElement>>, {
...
let found = if let Some(list_element) = self.list_element.read().unwrap().iter().find(|stored| {
if let Some(stored) = stored {
...
(0..len).filter(|i| self.list_element.read().unwrap()[*i].is_some()).map(move|i|{self.list_element.read().unwrap()[i].as_ref().map(|list_element| list_element.clone()).unwrap()})
}

/// Inter (insert) [`ListExpression`] into the store.
///
#[inline]
pub fn inter_list_expression<F>(&mut self, list_expression: F) -> Arc<RwLock<ListExpression>> where F: Fn(usize) -> Arc<RwLock<ListExpression>>, {
...
let found = if let Some(list_expression) = self.list_expression.read().unwrap().iter().find(|stored| {
if let Some(stored) = stored {
...
(0..len).filter(|i| self.list_expression.read().unwrap()[*i].is_some()).map(move|i|{self.list_expression.read().unwrap()[i].as_ref().map(|list_expression| list_expression.clone()).unwrap()})
}

/// Inter (insert) [`Literal`] into the store.
///
#[inline]
pub fn inter_literal<F>(&mut self, literal: F) -> Arc<RwLock<Literal>> where F: Fn(usize) -> Arc<RwLock<Literal>>, {
...
(0..len).filter(|i| self.literal.read().unwrap()[*i].is_some()).map(move|i|{self.literal.read().unwrap()[i].as_ref().map(|literal| literal.clone()).unwrap()})
}

/// Inter (insert) [`LocalVariable`] into the store.
///
#[inline]
pub fn inter_local_variable<F>(&mut self, local_variable: F) -> Arc<RwLock<LocalVariable>> where F: Fn(usize) -> Arc<RwLock<LocalVariable>>, {
...
let found = if let Some(local_variable) = self.local_variable.read().unwrap().iter().find(|stored| {
if let Some(stored) = stored {
...
(0..len).filter(|i| self.local_variable.read().unwrap()[*i].is_some()).map(move|i|{self.local_variable.read().unwrap()[i].as_ref().map(|local_variable| local_variable.clone()).unwrap()})
}

/// Inter (insert) [`XMacro`] into the store.
///
#[inline]
pub fn inter_x_macro<F>(&mut self, x_macro: F) -> Arc<RwLock<XMacro>> where F: Fn(usize) -> Arc<RwLock<XMacro>>, {
...
(0..len).filter(|i| self.x_macro.read().unwrap()[*i].is_some()).map(move|i|{self.x_macro.read().unwrap()[i].as_ref().map(|x_macro| x_macro.clone()).unwrap()})
}

/// Inter (insert) [`Map`] into the store.
///
#[inline]
pub fn inter_map<F>(&mut self, map: F) -> Arc<RwLock<Map>> where F: Fn(usize) -> Arc<RwLock<Map>>, {
...
(0..len).filter(|i| self.map.read().unwrap()[*i].is_some()).map(move|i|{self.map.read().unwrap()[i].as_ref().map(|map| map.clone()).unwrap()})
}

/// Inter (insert) [`MapElement`] into the store.
///
#[inline]
pub fn inter_map_element<F>(&mut self, map_element: F) -> Arc<RwLock<MapElement>> where F: Fn(usize) -> Arc<RwLock<MapElement>>, {
...
let found = if let Some(map_element) = self.map_element.read().unwrap().iter().find(|stored| {
if let Some(stored) = stored {
...
(0..len).filter(|i| self.map_element.read().unwrap()[*i].is_some()).map(move|i|{self.map_element.read().unwrap()[i].as_ref().map(|map_element| map_element.clone()).unwrap()})
}

/// Inter (insert) [`MapExpression`] into the store.
///
#[inline]
pub fn inter_map_expression<F>(&mut self, map_expression: F) -> Arc<RwLock<MapExpression>> where F: Fn(usize) -> Arc<RwLock<MapExpression>>, {
...
let found = if let Some(map_expression) = self.map_expression.read().unwrap().iter().find(|stored| {
if let Some(stored) = stored {
...
(0..len).filter(|i| self.map_expression.read().unwrap()[*i].is_some()).map(move|i|{self.map_expression.read().unwrap()[i].as_ref().map(|map_expression| map_expression.clone()).unwrap()})
}

/// Inter (insert) [`XMatch`] into the store.
///
#[inline]
pub fn inter_x_match<F>(&mut self, x_match: F) -> Arc<RwLock<XMatch>> where F: Fn(usize) -> Arc<RwLock<XMatch>>, {
...
(0..len).filter(|i| self.x_match.read().unwrap()[*i].is_some()).map(move|i|{self.x_match.read().unwrap()[i].as_ref().map(|x_match| x_match.clone()).unwrap()})
}

/// Inter (insert) [`MethodCall`] into the store.
///
#[inline]
pub fn inter_method_call<F>(&mut self, method_call: F) -> Arc<RwLock<MethodCall>> where F: Fn(usize) -> Arc<RwLock<MethodCall>>, {
...
let found = if let Some(method_call) = self.method_call.read().unwrap().iter().find(|stored| {
if let Some(stored) = stored {
...
(0..len).filter(|i| self.method_call.read().unwrap()[*i].is_some()).map(move|i|{self.method_call.read().unwrap()[i].as_ref().map(|method_call| method_call.clone()).unwrap()})
}

/// Inter (insert) [`NamedFieldExpression`] into the store.
///
#[inline]
pub fn inter_named_field_expression<F>(&mut self, named_field_expression: F) -> Arc<RwLock<NamedFieldExpression>> where F: Fn(usize) -> Arc<RwLock<NamedFieldExpression>>, {
let _index = if let Some(_index) = self.named_field_expression_free_list.lock().unwrap().pop() {
tracing::trace!(target: "store", "recycling block {_index}.");
_index
} else {
...
let found = if let Some(named_field_expression) = self.named_field_expression.read().unwrap().iter().find(|stored| {
if let Some(stored) = stored {
...
self.named_field_expression_free_list.lock().unwrap().push(_index);
named_field_expression.clone()
} else {
tracing::debug!(target: "store", "interring {named_field_expression:?}.");
self.named_field_expression.write().unwrap()[_index] = Some(named_field_expression.clone());
...
///
#[inline]
pub fn exhume_named_field_expression(&self, id: &usize) -> Option<Arc<RwLock<NamedFieldExpression>>> {
...
///
#[inline]
pub fn exorcise_named_field_expression(&mut self, id: &usize) -> Option<Arc<RwLock<NamedFieldExpression>>> {
...
self.named_field_expression_free_list.lock().unwrap().push(*id);
result
}
...
///
#[inline]
pub fn iter_named_field_expression(&self) -> impl Iterator<Item = Arc<RwLock<NamedFieldExpression>>> + '_ {
let len = self.named_field_expression.read().unwrap().len();
(0..len).filter(|i| self.named_field_expression.read().unwrap()[*i].is_some()).map(move|i|{self.named_field_expression.read().unwrap()[i].as_ref().map(|named_field_expression| named_field_expression.clone()).unwrap()})
}

/// Inter (insert) [`ZObjectStore`] into the store.
///
#[inline]
pub fn inter_z_object_store<F>(&mut self, z_object_store: F) -> Arc<RwLock<ZObjectStore>> where F: Fn(usize) -> Arc<RwLock<ZObjectStore>>, {
...
let found = if let Some(z_object_store) = self.z_object_store.read().unwrap().iter().find(|stored| {
if let Some(stored) = stored {
...
;
self.z_object_store_id_by_name.write().unwrap().insert(z_object_store.read().unwrap().name.to_owned(), z_object_store.read().unwrap().id);
z_object_store
}
...
self.z_object_store_id_by_name.read().unwrap().get(name).map(|z_object_store| *z_object_store)
}
...
(0..len).filter(|i| self.z_object_store.read().unwrap()[*i].is_some()).map(move|i|{self.z_object_store.read().unwrap()[i].as_ref().map(|z_object_store| z_object_store.clone()).unwrap()})
}

/// Inter (insert) [`ObjectWrapper`] into the store.
///
#[inline]
pub fn inter_object_wrapper<F>(&mut self, object_wrapper: F) -> Arc<RwLock<ObjectWrapper>> where F: Fn(usize) -> Arc<RwLock<ObjectWrapper>>, {
...
let found = if let Some(object_wrapper) = self.object_wrapper.read().unwrap().iter().find(|stored| {
if let Some(stored) = stored {
...
(0..len).filter(|i| self.object_wrapper.read().unwrap()[*i].is_some()).map(move|i|{self.object_wrapper.read().unwrap()[i].as_ref().map(|object_wrapper| object_wrapper.clone()).unwrap()})
}

/// Inter (insert) [`Operator`] into the store.
///
#[inline]
pub fn inter_operator<F>(&mut self, operator: F) -> Arc<RwLock<Operator>> where F: Fn(usize) -> Arc<RwLock<Operator>>, {
...
(0..len).filter(|i| self.operator.read().unwrap()[*i].is_some()).map(move|i|{self.operator.read().unwrap()[i].as_ref().map(|operator| operator.clone()).unwrap()})
}

/// Inter (insert) [`Parameter`] into the store.
///
#[inline]
pub fn inter_parameter<F>(&mut self, parameter: F) -> Arc<RwLock<Parameter>> where F: Fn(usize) -> Arc<RwLock<Parameter>>, {
...
(0..len).filter(|i| self.parameter.read().unwrap()[*i].is_some()).map(move|i|{self.parameter.read().unwrap()[i].as_ref().map(|parameter| parameter.clone()).unwrap()})
}

/// Inter (insert) [`XPath`] into the store.
///
#[inline]
pub fn inter_x_path<F>(&mut self, x_path: F) -> Arc<RwLock<XPath>> where F: Fn(usize) -> Arc<RwLock<XPath>>, {
...
(0..len).filter(|i| self.x_path.read().unwrap()[*i].is_some()).map(move|i|{self.x_path.read().unwrap()[i].as_ref().map(|x_path| x_path.clone()).unwrap()})
}

/// Inter (insert) [`PathElement`] into the store.
///
#[inline]
pub fn inter_path_element<F>(&mut self, path_element: F) -> Arc<RwLock<PathElement>> where F: Fn(usize) -> Arc<RwLock<PathElement>>, {
...
let found = if let Some(path_element) = self.path_element.read().unwrap().iter().find(|stored| {
if let Some(stored) = stored {
...
(0..len).filter(|i| self.path_element.read().unwrap()[*i].is_some()).map(move|i|{self.path_element.read().unwrap()[i].as_ref().map(|path_element| path_element.clone()).unwrap()})
}

/// Inter (insert) [`Pattern`] into the store.
///
#[inline]
pub fn inter_pattern<F>(&mut self, pattern: F) -> Arc<RwLock<Pattern>> where F: Fn(usize) -> Arc<RwLock<Pattern>>, {
...
(0..len).filter(|i| self.pattern.read().unwrap()[*i].is_some()).map(move|i|{self.pattern.read().unwrap()[i].as_ref().map(|pattern| pattern.clone()).unwrap()})
}

/// Inter (insert) [`XPlugin`] into the store.
///
#[inline]
pub fn inter_x_plugin<F>(&mut self, x_plugin: F) -> Arc<RwLock<XPlugin>> where F: Fn(usize) -> Arc<RwLock<XPlugin>>, {
...
;
self.x_plugin_id_by_name.write().unwrap().insert(x_plugin.read().unwrap().name.to_owned(), x_plugin.read().unwrap().id);
x_plugin
}
...
self.x_plugin_id_by_name.read().unwrap().get(name).map(|x_plugin| *x_plugin)
}
...
(0..len).filter(|i| self.x_plugin.read().unwrap()[*i].is_some()).map(move|i|{self.x_plugin.read().unwrap()[i].as_ref().map(|x_plugin| x_plugin.clone()).unwrap()})
}

/// Inter (insert) [`XPrint`] into the store.
///
#[inline]
pub fn inter_x_print<F>(&mut self, x_print: F) -> Arc<RwLock<XPrint>> where F: Fn(usize) -> Arc<RwLock<XPrint>>, {
...
(0..len).filter(|i| self.x_print.read().unwrap()[*i].is_some()).map(move|i|{self.x_print.read().unwrap()[i].as_ref().map(|x_print| x_print.clone()).unwrap()})
}

/// Inter (insert) [`RangeExpression`] into the store.
///
#[inline]
pub fn inter_range_expression<F>(&mut self, range_expression: F) -> Arc<RwLock<RangeExpression>> where F: Fn(usize) -> Arc<RwLock<RangeExpression>>, {
...
let found = if let Some(range_expression) = self.range_expression.read().unwrap().iter().find(|stored| {
if let Some(stored) = stored {
...
///
#[inline]
pub fn exorcise_range_expression(&mut self, id: &usize) -> Option<Arc<RwLock<RangeExpression>>> {
...
(0..len).filter(|i| self.range_expression.read().unwrap()[*i].is_some()).map(move|i|{self.range_expression.read().unwrap()[i].as_ref().map(|range_expression| range_expression.clone()).unwrap()})
}

/// Inter (insert) [`ResultStatement`] into the store.
///
#[inline]
pub fn inter_result_statement<F>(&mut self, result_statement: F) -> Arc<RwLock<ResultStatement>> where F: Fn(usize) -> Arc<RwLock<ResultStatement>>, {
...
let found = if let Some(result_statement) = self.result_statement.read().unwrap().iter().find(|stored| {
if let Some(stored) = stored {
...
///
#[inline]
pub fn exorcise_result_statement(&mut self, id: &usize) -> Option<Arc<RwLock<ResultStatement>>> {
...
(0..len).filter(|i| self.result_statement.read().unwrap()[*i].is_some()).map(move|i|{self.result_statement.read().unwrap()[i].as_ref().map(|result_statement| result_statement.clone()).unwrap()})
}

/// Inter (insert) [`XReturn`] into the store.
///
#[inline]
pub fn inter_x_return<F>(&mut self, x_return: F) -> Arc<RwLock<XReturn>> where F: Fn(usize) -> Arc<RwLock<XReturn>>, {
...
(0..len).filter(|i| self.x_return.read().unwrap()[*i].is_some()).map(move|i|{self.x_return.read().unwrap()[i].as_ref().map(|x_return| x_return.clone()).unwrap()})
}

/// Inter (insert) [`Span`] into the store.
///
#[inline]
pub fn inter_span<F>(&mut self, span: F) -> Arc<RwLock<Span>> where F: Fn(usize) -> Arc<RwLock<Span>>, {
...
(0..len).filter(|i| self.span.read().unwrap()[*i].is_some()).map(move|i|{self.span.read().unwrap()[i].as_ref().map(|span| span.clone()).unwrap()})
}

/// Inter (insert) [`Statement`] into the store.
///
#[inline]
pub fn inter_statement<F>(&mut self, statement: F) -> Arc<RwLock<Statement>> where F: Fn(usize) -> Arc<RwLock<Statement>>, {
...
(0..len).filter(|i| self.statement.read().unwrap()[*i].is_some()).map(move|i|{self.statement.read().unwrap()[i].as_ref().map(|statement| statement.clone()).unwrap()})
}

/// Inter (insert) [`StaticMethodCall`] into the store.
///
#[inline]
pub fn inter_static_method_call<F>(&mut self, static_method_call: F) -> Arc<RwLock<StaticMethodCall>> where F: Fn(usize) -> Arc<RwLock<StaticMethodCall>>, {
...
let found = if let Some(static_method_call) = self.static_method_call.read().unwrap().iter().find(|stored| {
if let Some(stored) = stored {
...
self.static_method_call_free_list.lock().unwrap().push(_index);
static_method_call.clone()
} else {
...
///
#[inline]
pub fn exorcise_static_method_call(&mut self, id: &usize) -> Option<Arc<RwLock<StaticMethodCall>>> {
...
///
#[inline]
pub fn iter_static_method_call(&self) -> impl Iterator<Item = Arc<RwLock<StaticMethodCall>>> + '_ {
let len = self.static_method_call.read().unwrap().len();
(0..len).filter(|i| self.static_method_call.read().unwrap()[*i].is_some()).map(move|i|{self.static_method_call.read().unwrap()[i].as_ref().map(|static_method_call| static_method_call.clone()).unwrap()})
}

/// Inter (insert) [`StringBit`] into the store.
///
#[inline]
pub fn inter_string_bit<F>(&mut self, string_bit: F) -> Arc<RwLock<StringBit>> where F: Fn(usize) -> Arc<RwLock<StringBit>>, {
...
let found = if let Some(string_bit) = self.string_bit.read().unwrap().iter().find(|stored| {
if let Some(stored) = stored {
...
(0..len).filter(|i| self.string_bit.read().unwrap()[*i].is_some()).map(move|i|{self.string_bit.read().unwrap()[i].as_ref().map(|string_bit| string_bit.clone()).unwrap()})
}

/// Inter (insert) [`StringLiteral`] into the store.
///
#[inline]
pub fn inter_string_literal<F>(&mut self, string_literal: F) -> Arc<RwLock<StringLiteral>> where F: Fn(usize) -> Arc<RwLock<StringLiteral>>, {
...
let found = if let Some(string_literal) = self.string_literal.read().unwrap().iter().find(|stored| {
if let Some(stored) = stored {
...
(0..len).filter(|i| self.string_literal.read().unwrap()[*i].is_some()).map(move|i|{self.string_literal.read().unwrap()[i].as_ref().map(|string_literal| string_literal.clone()).unwrap()})
}

/// Inter (insert) [`WoogStruct`] into the store.
///
#[inline]
pub fn inter_woog_struct<F>(&mut self, woog_struct: F) -> Arc<RwLock<WoogStruct>> where F: Fn(usize) -> Arc<RwLock<WoogStruct>>, {
...
let found = if let Some(woog_struct) = self.woog_struct.read().unwrap().iter().find(|stored| {
if let Some(stored) = stored {
...
;
self.woog_struct_id_by_name.write().unwrap().insert(woog_struct.read().unwrap().name.to_owned(), woog_struct.read().unwrap().id);
woog_struct
}
...
self.woog_struct_id_by_name.read().unwrap().get(name).map(|woog_struct| *woog_struct)
}
...
(0..len).filter(|i| self.woog_struct.read().unwrap()[*i].is_some()).map(move|i|{self.woog_struct.read().unwrap()[i].as_ref().map(|woog_struct| woog_struct.clone()).unwrap()})
}

/// Inter (insert) [`StructExpression`] into the store.
///
#[inline]
pub fn inter_struct_expression<F>(&mut self, struct_expression: F) -> Arc<RwLock<StructExpression>> where F: Fn(usize) -> Arc<RwLock<StructExpression>>, {
...
let found = if let Some(struct_expression) = self.struct_expression.read().unwrap().iter().find(|stored| {
if let Some(stored) = stored {
...
self.struct_expression_free_list.lock().unwrap().push(_index);
struct_expression.clone()
} else {
...
///
#[inline]
pub fn exorcise_struct_expression(&mut self, id: &usize) -> Option<Arc<RwLock<StructExpression>>> {
...
///
#[inline]
pub fn iter_struct_expression(&self) -> impl Iterator<Item = Arc<RwLock<StructExpression>>> + '_ {
let len = self.struct_expression.read().unwrap().len();
(0..len).filter(|i| self.struct_expression.read().unwrap()[*i].is_some()).map(move|i|{self.struct_expression.read().unwrap()[i].as_ref().map(|struct_expression| struct_expression.clone()).unwrap()})
}

/// Inter (insert) [`StructField`] into the store.
///
#[inline]
pub fn inter_struct_field<F>(&mut self, struct_field: F) -> Arc<RwLock<StructField>> where F: Fn(usize) -> Arc<RwLock<StructField>>, {
...
let found = if let Some(struct_field) = self.struct_field.read().unwrap().iter().find(|stored| {
if let Some(stored) = stored {
...
(0..len).filter(|i| self.struct_field.read().unwrap()[*i].is_some()).map(move|i|{self.struct_field.read().unwrap()[i].as_ref().map(|struct_field| struct_field.clone()).unwrap()})
}

/// Inter (insert) [`StructGeneric`] into the store.
///
#[inline]
pub fn inter_struct_generic<F>(&mut self, struct_generic: F) -> Arc<RwLock<StructGeneric>> where F: Fn(usize) -> Arc<RwLock<StructGeneric>>, {
...
let found = if let Some(struct_generic) = self.struct_generic.read().unwrap().iter().find(|stored| {
if let Some(stored) = stored {
...
(0..len).filter(|i| self.struct_generic.read().unwrap()[*i].is_some()).map(move|i|{self.struct_generic.read().unwrap()[i].as_ref().map(|struct_generic| struct_generic.clone()).unwrap()})
}

/// Inter (insert) [`TupleField`] into the store.
///
#[inline]
pub fn inter_tuple_field<F>(&mut self, tuple_field: F) -> Arc<RwLock<TupleField>> where F: Fn(usize) -> Arc<RwLock<TupleField>>, {
...
let found = if let Some(tuple_field) = self.tuple_field.read().unwrap().iter().find(|stored| {
if let Some(stored) = stored {
...
(0..len).filter(|i| self.tuple_field.read().unwrap()[*i].is_some()).map(move|i|{self.tuple_field.read().unwrap()[i].as_ref().map(|tuple_field| tuple_field.clone()).unwrap()})
}

/// Inter (insert) [`TypeCast`] into the store.
///
#[inline]
pub fn inter_type_cast<F>(&mut self, type_cast: F) -> Arc<RwLock<TypeCast>> where F: Fn(usize) -> Arc<RwLock<TypeCast>>, {
...
(0..len).filter(|i| self.type_cast.read().unwrap()[*i].is_some()).map(move|i|{self.type_cast.read().unwrap()[i].as_ref().map(|type_cast| type_cast.clone()).unwrap()})
}

/// Inter (insert) [`Unary`] into the store.
///
#[inline]
pub fn inter_unary<F>(&mut self, unary: F) -> Arc<RwLock<Unary>> where F: Fn(usize) -> Arc<RwLock<Unary>>, {
...
(0..len).filter(|i| self.unary.read().unwrap()[*i].is_some()).map(move|i|{self.unary.read().unwrap()[i].as_ref().map(|unary| unary.clone()).unwrap()})
}

/// Inter (insert) [`Unit`] into the store.
///
#[inline]
pub fn inter_unit<F>(&mut self, unit: F) -> Arc<RwLock<Unit>> where F: Fn(usize) -> Arc<RwLock<Unit>>, {
...
(0..len).filter(|i| self.unit.read().unwrap()[*i].is_some()).map(move|i|{self.unit.read().unwrap()[i].as_ref().map(|unit| unit.clone()).unwrap()})
}

/// Inter (insert) [`UnnamedFieldExpression`] into the store.
///
#[inline]
pub fn inter_unnamed_field_expression<F>(&mut self, unnamed_field_expression: F) -> Arc<RwLock<UnnamedFieldExpression>> where F: Fn(usize) -> Arc<RwLock<UnnamedFieldExpression>>, {
let _index = if let Some(_index) = self.unnamed_field_expression_free_list.lock().unwrap().pop() {
tracing::trace!(target: "store", "recycling block {_index}.");
_index
} else {
...
let found = if let Some(unnamed_field_expression) = self.unnamed_field_expression.read().unwrap().iter().find(|stored| {
if let Some(stored) = stored {
...
self.unnamed_field_expression_free_list.lock().unwrap().push(_index);
unnamed_field_expression.clone()
} else {
tracing::debug!(target: "store", "interring {unnamed_field_expression:?}.");
self.unnamed_field_expression.write().unwrap()[_index] = Some(unnamed_field_expression.clone());
...
///
#[inline]
pub fn exhume_unnamed_field_expression(&self, id: &usize) -> Option<Arc<RwLock<UnnamedFieldExpression>>> {
...
///
#[inline]
pub fn exorcise_unnamed_field_expression(&mut self, id: &usize) -> Option<Arc<RwLock<UnnamedFieldExpression>>> {
...
self.unnamed_field_expression_free_list.lock().unwrap().push(*id);
result
}
...
///
#[inline]
pub fn iter_unnamed_field_expression(&self) -> impl Iterator<Item = Arc<RwLock<UnnamedFieldExpression>>> + '_ {
let len = self.unnamed_field_expression.read().unwrap().len();
(0..len).filter(|i| self.unnamed_field_expression.read().unwrap()[*i].is_some()).map(move|i|{self.unnamed_field_expression.read().unwrap()[i].as_ref().map(|unnamed_field_expression| unnamed_field_expression.clone()).unwrap()})
}

/// Inter (insert) [`XValue`] into the store.
///
#[inline]
pub fn inter_x_value<F>(&mut self, x_value: F) -> Arc<RwLock<XValue>> where F: Fn(usize) -> Arc<RwLock<XValue>>, {
...
(0..len).filter(|i| self.x_value.read().unwrap()[*i].is_some()).map(move|i|{self.x_value.read().unwrap()[i].as_ref().map(|x_value| x_value.clone()).unwrap()})
}

/// Inter (insert) [`ValueType`] into the store.
///
#[inline]
pub fn inter_value_type<F>(&mut self, value_type: F) -> Arc<RwLock<ValueType>> where F: Fn(usize) -> Arc<RwLock<ValueType>>, {
...
let found = if let Some(value_type) = self.value_type.read().unwrap().iter().find(|stored| {
if let Some(stored) = stored {
...
(0..len).filter(|i| self.value_type.read().unwrap()[*i].is_some()).map(move|i|{self.value_type.read().unwrap()[i].as_ref().map(|value_type| value_type.clone()).unwrap()})
}

/// Inter (insert) [`Variable`] into the store.
///
#[inline]
pub fn inter_variable<F>(&mut self, variable: F) -> Arc<RwLock<Variable>> where F: Fn(usize) -> Arc<RwLock<Variable>>, {
...
(0..len).filter(|i| self.variable.read().unwrap()[*i].is_some()).map(move|i|{self.variable.read().unwrap()[i].as_ref().map(|variable| variable.clone()).unwrap()})
}

/// Inter (insert) [`VariableExpression`] into the store.
///
#[inline]
pub fn inter_variable_expression<F>(&mut self, variable_expression: F) -> Arc<RwLock<VariableExpression>> where F: Fn(usize) -> Arc<RwLock<VariableExpression>>, {
...
let found = if let Some(variable_expression) = self.variable_expression.read().unwrap().iter().find(|stored| {
if let Some(stored) = stored {
...
self.variable_expression_free_list.lock().unwrap().push(_index);
variable_expression.clone()
} else {
...
///
#[inline]
pub fn exhume_variable_expression(&self, id: &usize) -> Option<Arc<RwLock<VariableExpression>>> {
...
///
#[inline]
pub fn exorcise_variable_expression(&mut self, id: &usize) -> Option<Arc<RwLock<VariableExpression>>> {
...
///
#[inline]
pub fn iter_variable_expression(&self) -> impl Iterator<Item = Arc<RwLock<VariableExpression>>> + '_ {
let len = self.variable_expression.read().unwrap().len();
(0..len).filter(|i| self.variable_expression.read().unwrap()[*i].is_some()).map(move|i|{self.variable_expression.read().unwrap()[i].as_ref().map(|variable_expression| variable_expression.clone()).unwrap()})
}
