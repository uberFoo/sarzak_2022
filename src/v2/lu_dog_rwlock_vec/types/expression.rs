...
use crate::v2::lu_dog_rwlock_vec::types::empty_expression::EMPTY_EXPRESSION;
use crate::v2::lu_dog_rwlock_vec::types::expression_bit::ExpressionBit;
...
use crate::v2::lu_dog_rwlock_vec::types::halt_and_catch_fire::HaltAndCatchFire;
...
use crate::v2::lu_dog_rwlock_vec::types::x_debugger::X_DEBUGGER;
...
impl Expression {
    ...
    /// Inter a new Expression in the store, and return it's `id`.
    pub fn new_x_debugger(bogus: bool, store: &mut LuDogRwlockVecStore) -> Arc<RwLock<Expression>> {
        store.inter_expression(|id| {
            Arc::new(RwLock::new(Expression {
                bogus: bogus,
    ...
    pub fn new_halt_and_catch_fire(
        bogus: bool,
        subtype: &Arc<RwLock<HaltAndCatchFire>>,
        store: &mut LuDogRwlockVecStore,
    ) -> Arc<RwLock<Expression>> {
        store.inter_expression(|id| {
            Arc::new(RwLock::new(Expression {
                bogus: bogus,
                subtype: ExpressionEnum::HaltAndCatchFire(subtype.read().unwrap().id), // b
                id,
            }))
        })
    }
    ...
    pub fn r114_halt_and_catch_fire<'a>(
        &'a self,
        store: &'a LuDogRwlockVecStore,
    ) -> Vec<Arc<RwLock<HaltAndCatchFire>>> {
        store
            .iter_halt_and_catch_fire()
            .filter(|halt_and_catch_fire| halt_and_catch_fire.read().unwrap().expression == self.id)
            .collect()
    }
    // {"magic":"","directive":{"End":{"directive":"ignore-orig"}}}
    ...
    pub fn r118_map_element<'a>(
        ...
        store
            .iter_map_element()
            .filter(|map_element| map_element.read().unwrap().x_value == self.id)
            .collect()
    }
    // {"magic":"","directive":{"End":{"directive":"ignore-orig"}}}
    // {"magic":"","directive":{"Start":{"directive":"ignore-orig","tag":"expression-struct-impl-nav-backward-1_M-to-map_element"}}}
    ...
    pub fn r116_map_element<'a>(
        ...
        store
            .iter_map_element()
            .filter(|map_element| map_element.read().unwrap().key == self.id)
            .collect()
    }
    // {"magic":"","directive":{"End":{"directive":"ignore-orig"}}}
    ...
    pub fn r32_x_print<'a>(&'a self, store: &'a LuDogRwlockVecStore) -> Vec<Arc<RwLock<XPrint>>> {
        store
            .iter_x_print()
    ...
    pub fn r87_pattern<'a>(&'a self, store: &'a LuDogRwlockVecStore) -> Vec<Arc<RwLock<Pattern>>> {
        let pattern = store
            .iter_pattern()
            .find(|pattern| pattern.read().unwrap().match_expr == self.id);
        match pattern {
