...
impl XValue {
    ...
    pub fn r258_x_value<'a>(&'a self, store: &'a LuDogRwlockVecStore) -> Vec<Arc<RwLock<XValue>>> {
        match self.next {
    ...
    pub fn r258c_x_value<'a>(&'a self, store: &'a LuDogRwlockVecStore) -> Vec<Arc<RwLock<XValue>>> {
        let x_value = store
            .iter_x_value()
