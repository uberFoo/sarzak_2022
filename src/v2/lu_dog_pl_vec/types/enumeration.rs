...
impl Enumeration {
    ...
    pub fn r88_enum_field<'a>(&'a self, store: &'a LuDogPlVecStore) -> Vec<Arc<RwLock<EnumField>>> {
        store
            .iter_enum_field()
            .filter(|enum_field| enum_field.read().woog_enum == self.id)
            .collect()
    }
    // {"magic":"","directive":{"End":{"directive":"ignore-orig"}}}
    ...
    pub fn r104_enum_generic<'a>(
        ...
        store
            .iter_enum_generic()
            .filter(|enum_generic| enum_generic.read().woog_enum == self.id)
            .collect()
    }
    // {"magic":"","directive":{"End":{"directive":"ignore-orig"}}}
    ...
    pub fn r120_enum_generic_type<'a>(
        ...
        vec![store
            ...
            .find(|enum_generic_type| enum_generic_type.read().enumeration == self.id)
            .unwrap()]
    }
    // {"magic":"","directive":{"End":{"directive":"ignore-orig"}}}
    ...
    pub fn r95_data_structure<'a>(
        ...
        vec![store
            .iter_data_structure()
            .find(|data_structure| {
                if let DataStructureEnum::Enumeration(id) = data_structure.read().subtype {
                    id == self.id
                } else {
                    false
                }
    ...
    pub fn r6_item<'a>(&'a self, store: &'a LuDogPlVecStore) -> Vec<Arc<RwLock<Item>>> {
        vec![store
            .iter_item()
            .find(|item| {
                if let ItemEnum::Enumeration(id) = item.read().subtype {
                    id == self.id
                } else {
                    false
                }
    ...
    pub fn r1_value_type<'a>(&'a self, store: &'a LuDogPlVecStore) -> Vec<Arc<RwLock<ValueType>>> {
        vec![store
            .iter_value_type()
            .find(|value_type| {
                if let ValueTypeEnum::Enumeration(id) = value_type.read().subtype {
                    id == self.id
                } else {
                    false
                }
