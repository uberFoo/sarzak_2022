...
use crate::v2::lu_dog_pl_vec::types::any_list::AnyList;
...
use crate::v2::lu_dog_pl_vec::types::enum_generic_type::EnumGenericType;
use crate::v2::lu_dog_pl_vec::types::enumeration::Enumeration;
use crate::v2::lu_dog_pl_vec::types::field::Field;
use crate::v2::lu_dog_pl_vec::types::func_generic::FuncGeneric;
...
use crate::v2::lu_dog_pl_vec::types::lambda_parameter::LambdaParameter;
use crate::v2::lu_dog_pl_vec::types::list::List;
use crate::v2::lu_dog_pl_vec::types::map::Map;
use crate::v2::lu_dog_pl_vec::types::parameter::Parameter;
use crate::v2::lu_dog_pl_vec::types::range::RANGE;
use crate::v2::lu_dog_pl_vec::types::span::Span;
use crate::v2::lu_dog_pl_vec::types::struct_generic::StructGeneric;
use crate::v2::lu_dog_pl_vec::types::task::TASK;
use crate::v2::lu_dog_pl_vec::types::tuple_field::TupleField;
use crate::v2::lu_dog_pl_vec::types::type_cast::TypeCast;
use crate::v2::lu_dog_pl_vec::types::unknown::UNKNOWN;
use crate::v2::lu_dog_pl_vec::types::woog_struct::WoogStruct;
use crate::v2::lu_dog_pl_vec::types::x_future::XFuture;
use crate::v2::lu_dog_pl_vec::types::x_plugin::XPlugin;
...
impl ValueType {
    ...
    pub fn new_any_list(
        ...
        store.inter_value_type(|id| {
            Arc::new(RwLock::new(ValueType {
                bogus: bogus,
                subtype: ValueTypeEnum::AnyList(subtype.read().id), // b
                id,
            }))
        })
    }
    ...
    /// Inter a new ValueType in the store, and return it's `id`.
    pub fn new_empty(bogus: bool, store: &mut LuDogPlVecStore) -> Arc<RwLock<ValueType>> {
        store.inter_value_type(|id| {
            Arc::new(RwLock::new(ValueType {
                bogus: bogus,
    ...
    pub fn new_enum_generic(
        ...
        store.inter_value_type(|id| {
            Arc::new(RwLock::new(ValueType {
                bogus: bogus,
                subtype: ValueTypeEnum::EnumGeneric(subtype.read().id), // b
                id,
            }))
        })
    }
    ...
    pub fn new_enumeration(
        ...
        store.inter_value_type(|id| {
            Arc::new(RwLock::new(ValueType {
                bogus: bogus,
                subtype: ValueTypeEnum::Enumeration(subtype.read().id), // b
                id,
            }))
        })
    }
    ...
    pub fn new_func_generic(
        ...
        store.inter_value_type(|id| {
            Arc::new(RwLock::new(ValueType {
                bogus: bogus,
                subtype: ValueTypeEnum::FuncGeneric(subtype.read().id), // b
                id,
            }))
        })
    }
    ...
    pub fn new_function(
        ...
        store.inter_value_type(|id| {
            Arc::new(RwLock::new(ValueType {
                bogus: bogus,
                subtype: ValueTypeEnum::Function(subtype.read().id), // b
                id,
            }))
        })
    }
    ...
    pub fn new_x_future(
        ...
        store.inter_value_type(|id| {
            Arc::new(RwLock::new(ValueType {
                bogus: bogus,
                subtype: ValueTypeEnum::XFuture(subtype.read().id), // b
                id,
            }))
        })
    }
    ...
    pub fn new_import(
        ...
        store.inter_value_type(|id| {
            Arc::new(RwLock::new(ValueType {
                bogus: bogus,
                subtype: ValueTypeEnum::Import(subtype.read().id), // b
                id,
            }))
        })
    }
    ...
    pub fn new_lambda(
        ...
        store.inter_value_type(|id| {
            Arc::new(RwLock::new(ValueType {
                bogus: bogus,
                subtype: ValueTypeEnum::Lambda(subtype.read().id), // b
                id,
            }))
        })
    }
    ...
    pub fn new_list(
        ...
        store.inter_value_type(|id| {
            Arc::new(RwLock::new(ValueType {
                bogus: bogus,
                subtype: ValueTypeEnum::List(subtype.read().id), // b
                id,
            }))
        })
    }
    ...
    pub fn new_map(
        ...
        store.inter_value_type(|id| {
            Arc::new(RwLock::new(ValueType {
                bogus: bogus,
                subtype: ValueTypeEnum::Map(subtype.read().id), // b
                id,
            }))
        })
    }
    ...
    pub fn new_z_object_store(
        ...
        store.inter_value_type(|id| {
            Arc::new(RwLock::new(ValueType {
                bogus: bogus,
                subtype: ValueTypeEnum::ZObjectStore(subtype.read().id), // b
                id,
            }))
        })
    }
    ...
    pub fn new_x_plugin(
        ...
        store.inter_value_type(|id| {
            Arc::new(RwLock::new(ValueType {
                bogus: bogus,
                subtype: ValueTypeEnum::XPlugin(subtype.read().id), // b
                id,
            }))
        })
    }
    ...
    /// Inter a new ValueType in the store, and return it's `id`.
    pub fn new_range(bogus: bool, store: &mut LuDogPlVecStore) -> Arc<RwLock<ValueType>> {
        store.inter_value_type(|id| {
            Arc::new(RwLock::new(ValueType {
                bogus: bogus,
    ...
    pub fn new_woog_struct(
        ...
        store.inter_value_type(|id| {
            Arc::new(RwLock::new(ValueType {
                bogus: bogus,
                subtype: ValueTypeEnum::WoogStruct(subtype.read().id), // b
                id,
            }))
        })
    }
    ...
    pub fn new_struct_generic(
        ...
        store.inter_value_type(|id| {
            Arc::new(RwLock::new(ValueType {
                bogus: bogus,
                subtype: ValueTypeEnum::StructGeneric(subtype.read().id), // b
                id,
            }))
        })
    }
    ...
    /// Inter a new ValueType in the store, and return it's `id`.
    pub fn new_task(bogus: bool, store: &mut LuDogPlVecStore) -> Arc<RwLock<ValueType>> {
        store.inter_value_type(|id| {
            Arc::new(RwLock::new(ValueType {
                bogus: bogus,
    ...
    /// Inter a new ValueType in the store, and return it's `id`.
    pub fn new_unknown(bogus: bool, store: &mut LuDogPlVecStore) -> Arc<RwLock<ValueType>> {
        store.inter_value_type(|id| {
            Arc::new(RwLock::new(ValueType {
                bogus: bogus,
    ...
    pub fn r119_enum_generic_type<'a>(
        ...
        store
            ...
            .filter(|enum_generic_type| enum_generic_type.read().ty == self.id)
            .collect()
    }
    // {"magic":"","directive":{"End":{"directive":"ignore-orig"}}}
    ...
    pub fn r5_field<'a>(&'a self, store: &'a LuDogPlVecStore) -> Vec<Arc<RwLock<Field>>> {
        store
            .iter_field()
    ...
    pub fn r10_function<'a>(&'a self, store: &'a LuDogPlVecStore) -> Vec<Arc<RwLock<Function>>> {
        store
            .iter_function()
    ...
    pub fn r2_x_future<'a>(&'a self, store: &'a LuDogPlVecStore) -> Vec<Arc<RwLock<XFuture>>> {
        store
            .iter_x_future()
            .filter(|x_future| x_future.read().x_value == self.id)
            .collect()
    }
    // {"magic":"","directive":{"End":{"directive":"ignore-orig"}}}
    ...
    pub fn r74_lambda<'a>(&'a self, store: &'a LuDogPlVecStore) -> Vec<Arc<RwLock<Lambda>>> {
        store
            .iter_lambda()
            .filter(|lambda| lambda.read().return_type == self.id)
            .collect()
    }
    // {"magic":"","directive":{"End":{"directive":"ignore-orig"}}}
    ...
    pub fn r77_lambda_parameter<'a>(
        ...
        store
            ...
            .filter(|lambda_parameter| lambda_parameter.read().ty == Some(self.id))
            .collect()
    }
    // {"magic":"","directive":{"End":{"directive":"ignore-orig"}}}
    ...
    pub fn r36_list<'a>(&'a self, store: &'a LuDogPlVecStore) -> Vec<Arc<RwLock<List>>> {
        store
            .iter_list()
    ...
    pub fn r115_map<'a>(&'a self, store: &'a LuDogPlVecStore) -> Vec<Arc<RwLock<Map>>> {
        store
            .iter_map()
            .filter(|map| map.read().key_type == self.id)
            .collect()
    }
    // {"magic":"","directive":{"End":{"directive":"ignore-orig"}}}
    // {"magic":"","directive":{"Start":{"directive":"ignore-orig","tag":"value_type-struct-impl-nav-backward-1_M-to-map"}}}
    /// Navigate to [`Map`] across R116(1-M)
    pub fn r116_map<'a>(&'a self, store: &'a LuDogPlVecStore) -> Vec<Arc<RwLock<Map>>> {
        store
            .iter_map()
            .filter(|map| map.read().value_type == self.id)
            .collect()
    }
    // {"magic":"","directive":{"End":{"directive":"ignore-orig"}}}
    ...
    pub fn r79_parameter<'a>(&'a self, store: &'a LuDogPlVecStore) -> Vec<Arc<RwLock<Parameter>>> {
        store
            .iter_parameter()
            .filter(|parameter| parameter.read().ty == self.id)
            .collect()
    }
    // {"magic":"","directive":{"End":{"directive":"ignore-orig"}}}
    ...
    pub fn r62_span<'a>(&'a self, store: &'a LuDogPlVecStore) -> Vec<Arc<RwLock<Span>>> {
        store
            .iter_span()
            .filter(|span| span.read().ty == Some(self.id))
            .collect()
    }
    // {"magic":"","directive":{"End":{"directive":"ignore-orig"}}}
    ...
    pub fn r86_tuple_field<'a>(
        ...
        store
            .iter_tuple_field()
            .filter(|tuple_field| tuple_field.read().ty == self.id)
            .collect()
    }
    // {"magic":"","directive":{"End":{"directive":"ignore-orig"}}}
    ...
    pub fn r69_type_cast<'a>(&'a self, store: &'a LuDogPlVecStore) -> Vec<Arc<RwLock<TypeCast>>> {
        store
            .iter_type_cast()
    ...
    pub fn r24_x_value<'a>(&'a self, store: &'a LuDogPlVecStore) -> Vec<Arc<RwLock<XValue>>> {
        store
            .iter_x_value()
