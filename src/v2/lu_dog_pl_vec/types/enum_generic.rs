...
impl EnumGeneric {
    ...
    pub fn new(
        ...
        store.inter_enum_generic(|id| {
            Arc::new(RwLock::new(EnumGeneric {
                ...
                next: next.map(|enum_generic| enum_generic.read().id),
            }))
        })
    }
    // {"magic":"","directive":{"End":{"directive":"ignore-orig"}}}
    ...
    pub fn r106c_enum_generic<'a>(
        ...
        let enum_generic = store
            .iter_enum_generic()
            .find(|enum_generic| enum_generic.read().next == Some(self.id));
        match enum_generic {
    ...
    pub fn r105_enumeration<'a>(
        ...
        vec![store
            .iter_enumeration()
            .find(|enumeration| enumeration.read().first_generic == Some(self.id))
            .unwrap()]
    }
    // {"magic":"","directive":{"End":{"directive":"ignore-orig"}}}
    ...
    pub fn r120_enum_generic_type<'a>(
        ...
        store
            ...
            .filter(|enum_generic_type| enum_generic_type.read().generic == self.id)
            .collect()
    }
    // {"magic":"","directive":{"End":{"directive":"ignore-orig"}}}
    ...
    pub fn r1_value_type<'a>(&'a self, store: &'a LuDogPlVecStore) -> Vec<Arc<RwLock<ValueType>>> {
        vec![store
            .iter_value_type()
            .find(|value_type| {
                if let ValueTypeEnum::EnumGeneric(id) = value_type.read().subtype {
                    id == self.id
                } else {
                    false
                }
