...
use crate::v2::lu_dog_pl_vec::types::halt_and_catch_fire::HaltAndCatchFire;
...
impl Expression {
    ...
    pub fn new_halt_and_catch_fire(
        ...
        store.inter_expression(|id| {
            Arc::new(RwLock::new(Expression {
                bogus: bogus,
                subtype: ExpressionEnum::HaltAndCatchFire(subtype.read().id), // b
                id,
            }))
        })
    }
    ...
    pub fn r114_halt_and_catch_fire<'a>(
        ...
        store
            ...
            .filter(|halt_and_catch_fire| halt_and_catch_fire.read().expression == self.id)
            .collect()
    }
    // {"magic":"","directive":{"End":{"directive":"ignore-orig"}}}
    ...
    pub fn r118_map_element<'a>(
        ...
        store
            .iter_map_element()
            .filter(|map_element| map_element.read().x_value == self.id)
            .collect()
    }
    // {"magic":"","directive":{"End":{"directive":"ignore-orig"}}}
    // {"magic":"","directive":{"Start":{"directive":"ignore-orig","tag":"expression-struct-impl-nav-backward-1_M-to-map_element"}}}
    ...
    pub fn r116_map_element<'a>(
        ...
        store
            .iter_map_element()
            .filter(|map_element| map_element.read().key == self.id)
            .collect()
    }
    // {"magic":"","directive":{"End":{"directive":"ignore-orig"}}}
